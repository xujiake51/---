...
% 图片路径：Q1 图在 output，Q2/Q3 图在 code
\graphicspath{{../output/}{../code/}}
...
% ============================================================
\begin{center}
  {\zihao{2}\bfseries 海上油田人员直升机运载计划编排}\\[1.2em]
...
出海与海返高度配对：对问题二数据的预分析显示，各设施出海与海返人数之差落在 $[-4,+6]$、48/52 个设施在 $\pm3$ 以内，即“送去多少人、几乎接回多少人”。受此启发，把满座部分配对为往返架次，去程满载送达、返程满载接回，一个架次兼负两程、使架次数近乎减半；只有真正新增的穿梭客流与未配对的少量尾数需要额外运力。

\subsection{问题三}

问题三在问题二路由结构不变的基础上，叠加时间窗与有限机队两重约束，是 VRPTW 与机队调度的结合。其关键结构特征是：

任务类型与窗口紧度高度相关：应急处置与增储上产共 400 人的窗口全部落在单天之内，最短者不足半小时，其航班被迫落在窗日、只能小群组直飞；常规倒班与临时任务共 3600 人的窗口跨 2--4 天，“哪天飞”才是真正的决策变量。据此把需求分为紧核 400 人与松壳 3600 人两层，前者单天固定、后者跨天可选，其求解顺序恰与题目“应急处置 $\to$ 增储上产 $\to$ 常规倒班 $\to$ 临时任务”的优先级一致。
...
用上述模型与算法求解，得到问题一的完整运载计划，核心指标见表~\ref{tab:q1res}，航线网络见图~\ref{fig:q1net}，求解结果综合对比见图~\ref{fig:q1sum}。
...
用上述模型与算法求解，得到问题二的完整运载计划，核心指标见表~\ref{tab:q2res}，航线网络见图~\ref{fig:q2net}，求解结果综合对比见图~\ref{fig:q2sum}。
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.72\textwidth]{问题二_求解结果综合图.png}
  \caption{问题二求解结果综合}
  \label{fig:q2sum}
\end{figure}
...
用上述模型与算法求解，得到问题三的完整运载计划，核心指标见表~\ref{tab:q3res}，机队排班见图~\ref{fig:q3fleet}，求解结果综合对比见图~\ref{fig:q3sum}。
...
\begin{figure}[H]
  ...
  \caption{问题三机队排班（24 架飞机 7 日）}
  \label{fig:q3fleet}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.72\textwidth]{问题三_求解结果综合图.png}
  \caption{问题三求解结果综合（指标与临时任务满足情况）}
  \label{fig:q3sum}
\end{figure}
...
\begin{enumerate}\sloppy
  ...
  \item 数据探针与可视化：\texttt{列数探针.py}、\texttt{探针Q1.py}、\texttt{探针Q2.py}、\texttt{探针Q2\_full.py}、\texttt{统计请求.py}、\texttt{热力图.py}；
  \item 可视化图片：\texttt{问题一\_航线网络图.png}、\texttt{问题一\_求解结果综合图.png}、\texttt{问题二\_航线网络图.png}、\texttt{问题二\_求解结果综合图.png}、\texttt{问题三\_机队排班图.png}、\texttt{问题三\_求解结果综合图.png} 等；
  \item AI 工具使用详情：\texttt{AI 工具使用详情.pdf}。
\end{enumerate}

\section{求解程序源代码}

问题一、问题二、问题三的完整可运行源程序如下（均可用 \texttt{python 文件名.py} 直接运行，代码以边框括起）：
